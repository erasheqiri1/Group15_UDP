...
server.on('message', (msg, rinfo) => {
    ...
    if (message.startsWith('HELLO')) {
        ...
        sendToClient(client, `WELCOME role=${client.role}. Use /list /read /upload /download ...`);
        return;
    }
    ...
    if (!client) {
        server.send(Buffer.from('ERROR Please send "HELLO" first.'), rinfo.port, rinfo.address);
        return;
    }
